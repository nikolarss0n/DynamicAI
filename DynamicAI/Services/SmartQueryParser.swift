...
struct SmartQueryParser {
    ...
    private func parseWithLLM(_ query: String) async throws -> ParsedPhotoQuery {
        ...
        let systemPrompt = """
        You are a photo search query parser. Extract structured information from natural language.
        ...
        Examples:
        "photos from my last trip" → location: null, labels: ["outdoor", "travel"], isMyPhotos: false
        "beach photos from Greece" → location: "Greece", labels: ["beach", "outdoor"], isMyPhotos: false
        "photos of me at the beach" → labels: ["beach"], isMyPhotos: true
        "sunset pictures last summer" → labels: ["sunset"], timePeriod: {start: "\(currentYear-1)-06-01", end: "\(currentYear-1)-08-31"}
        "show me recent photos from Sofia" → location: "Sofia, Bulgaria", labels: [], timePeriod: {description: "recent", startDate: null, endDate: null}
        """
